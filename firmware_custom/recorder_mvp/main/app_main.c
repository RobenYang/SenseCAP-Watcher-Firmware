...
void app_main(void)
{
    ...
    if (rec_power_init() != ESP_OK ||
        rec_storage_init() != ESP_OK ||
        rec_audio_init() != ESP_OK ||
        rec_input_init() != ESP_OK ||
        rec_led_init() != ESP_OK ||
        rec_ui_init() != ESP_OK) {
        rec_app_enter_error();
    }
    ...
    bool usb_attached_prev = rec_storage_is_usb_attached();
    ...
    while (1) {
        ...
        if (s_state == REC_STATE_RECORDING && rec_storage_has_error()) {
            rec_app_enter_error();
        }

        int64_t now_ms = esp_timer_get_time() / 1000;
        if ((now_ms - last_battery_ms) >= REC_APP_BATTERY_MS) {
            rec_ui_set_battery_percent(bsp_battery_get_percent());
