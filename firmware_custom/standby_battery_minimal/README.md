...
## 分支定位
- 本目录对应的固件是一个**低功耗基底**，不是一次性演示工程。
- 后续若要在此基础上增加联网、音频、传感器或 UI 功能，必须先确保新增版本的待机功耗与当前基底版本对齐，再按需求逐项增加功能并重新复测。
- 当前实测结论是：**续航仍可接受**，因此本基底可作为后续增量开发的起点。

## 文档入口
- 续航优化策略、约束、验证口径与官方文档引用，见 `firmware_custom/standby_battery_minimal/STANDBY_BASELINE.md`。
