...
- 其他扩展 IO 事件（例如 SD 卡插拔）不应再被误判成滚轮按键唤醒录音；若只是 SD 卡状态变化，设备会直接回到深度休眠。
- 从深睡唤醒后，优先顺序为：
  1. 初始化最小电源域与按键路径。
  2. 启动前面板录音指示灯闪烁。
  3. 启动音频采集并写入应用层缓冲。
...
- 深睡唤醒后会先对 `VBUS` 与 SD 检测脚做一次短延时复判，过滤掉插电和插卡导致的误唤醒。
- 当前版本按实测将 `VBUS_IN_DET` 视为低电平有效；若后续更换硬件批次，应重新复核该检测极性。
